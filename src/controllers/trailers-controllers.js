...
trailersCtrl.visualizar = async (req, res) => {
    ...
    await Trailer.findById(req.body.id)
        .then(tr=>{ res.json(tr)})
};
...
trailersCtrl.cargar =  async (req,res) => {
    ...
    if(trailer){
        return res.status(401).json({msg:'Ya se encuentra un trailer con el mismo título, intente con otro'})
    } else{
        const trailerD = await Trailer.findOne({titulo : req.body.descripcion}); 
        if(trailerD){
            return res.status(401).json({msg:'La descripción se encuentra en otro trailer, recuerde que debe ser única'})
        }  
    }
    ...
    if(req.libro){
        ...
        const libro= await Libro.findById(req.body.libro).updateOne({trailer: nuevoTrailer.__id});
...
/*    await Trailer.findByIdAndRemove(req.body.id)
        .then(res.send('Trailer eliminado')); 
*/    
    await trailer.findOneAndRemove().then(res.status(200).json({msg:'Trailer eliminado'}))
};
